const inputProjectBtn = document.getElementById('projectEnrollBtn'),
  ...
  projectUpdateBtn = document.getElementById('projectUpdateBtn'),
...
projectUpdateBtn.addEventListener('click', (event) => {
  ...
  fetch('./update', {
    ...
    .then((res) => {
      if (res.success) {
        ...
        location.href = `/${userId}/${project_id}`;
      } else {
        alert(res.msg);
      }
    })
